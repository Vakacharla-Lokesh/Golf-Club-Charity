    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-600">Draw Date: {new Date(draw.draw_date).toLocaleDateString()}</p>
        </div>
      ...
      {!simulation ? (
        <div className="rounded-lg bg-blue-50 border border-blue-200 p-6">
          ...
          <p className="mt-2 text-blue-800">
            Click below to run a draw simulation. This will generate winning numbers and show potential winners
        ...
        <div className="rounded-lg bg-green-50 border border-green-200 p-6">
          ...
          <p className="mt-2 text-green-800">
            Winning Numbers: <span className="font-mono font-bold text-lg">{simulation.winningNumbers.join(', ')}</span>
          </p>

          <div className="mt-4 grid grid-cols-2 gap-4 md:grid-cols-1">
            <div>
              <p className="text-sm text-green-700">Active Subscribers</p>
              <p className="text-2xl font-bold text-green-900">{simulation.totalSubscribers}</p>
            </div>
            <div>
              <p className="text-sm text-green-700">Total Winners</p>
              <p className="text-2xl font-bold text-green-900">{simulation.summary.totalWinners}</p>
            </div>
            <div>
              <p className="text-sm text-green-700">Tier 5 Winners (5 matches)</p>
              <p className="text-2xl font-bold text-yellow-600">{simulation.summary.tier5Count}</p>
            </div>
            <div>
              <p className="text-sm text-green-700">Tier 4 Winners (4 matches)</p>
              <p className="text-2xl font-bold text-blue-600">{simulation.summary.tier4Count}</p>
            </div>
            <div>
              <p className="text-sm text-green-700">Tier 3 Winners (3 matches)</p>
              <p className="text-2xl font-bold text-green-600">{simulation.summary.tier3Count}</p>
            </div>
          </div>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="px-4 py-2 text-center font-semibold text-gray-700">Scores</th>
                  <th className="px-4 py-2 text-center font-semibold text-gray-700">Matches</th>
                  <th className="px-4 py-2 text-center font-semibold text-gray-700">Prize Tier</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  .map((result) => (
                    <tr key={result.userId} className="border-b border-gray-100">
                      <td className="px-4 py-2 font-medium text-gray-900">{result.fullName}</td>
                      <td className="px-4 py-2 text-sm text-gray-600">{result.userEmail}</td>
          ...
          <div className="mt-4 p-4 rounded-lg bg-gray-50">
            <p className="text-sm text-gray-600">
              Total Winners: <span className="font-semibold">{simulation.summary.totalWinners}</span>
            </p>
            <p className="text-sm text-gray-600 mt-1">
              5-Match Winners: <span className="font-semibold">{simulation.summary.tier5Count}</span>
            </p>
            <p className="text-sm text-gray-600 mt-1">
              4-Match Winners: <span className="font-semibold">{simulation.summary.tier4Count}</span>
            </p>
            <p className="text-sm text-gray-600 mt-1">
              3-Match Winners: <span className="font-semibold">{simulation.summary.tier3Count}</span>
            </p>
          </div>
        </div>
      )}
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <p className="mt-1 text-2xl font-bold text-gray-900">{simulation.winningNumbers.join(', ')}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Total Subscribers</p>
              <p className="mt-1 text-2xl font-bold text-gray-900">{simulation.totalSubscribers}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Total Winners</p>
              <p className="mt-1 text-2xl font-bold text-blue-600">{simulation.summary.totalWinners}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Winner Percentage</p>
              <p className="mt-1 text-2xl font-bold text-blue-600">
                {simulation.totalSubscribers > 0
                  ? ((simulation.summary.totalWinners / simulation.totalSubscribers) * 100).toFixed(1)
                  : 0}
                %
              </p>
            </div>
