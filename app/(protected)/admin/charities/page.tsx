...
export default function AdminCharitiesPage() {
  ...
  const handleToggleFeatured = async (charityId: string, isFeatured: boolean) => {
    try {
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-2 text-gray-600">Add and manage charities supported on the platform.</p>
      </div>

      {/* Create New Charity */}
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Add New Charity</h3>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200">
              ...
              <th className="px-6 py-3 text-center text-sm font-semibold text-gray-900">Featured</th>
          ...
          <tbody>
            {charities.map((charity) => (
              <tr key={charity.id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="px-6 py-4 text-sm font-medium text-gray-900">{charity.name}</td>
                ...
                <td className="px-6 py-4 text-center">
                  <button
                    onClick={() => handleToggleFeatured(charity.id, charity.is_featured || false)}
